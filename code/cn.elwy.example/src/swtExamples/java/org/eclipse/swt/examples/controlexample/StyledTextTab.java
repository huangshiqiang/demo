...
class StyledTextTab extends ScrollableTab {
    ...
    void createExampleGroup() {
        ...
        styledTextGroup.setLayoutData(new GridData(GridData.GRAB_HORIZONTAL | GridData.HORIZONTAL_ALIGN_FILL
        ...
        boldButton.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                for (int i = sel.x; i < sel.x + sel.y; i++) {
                    ...
                    if (range == null) {
                        style = new StyleRange(i, 1, null, null, SWT.BOLD);
                    } else {
                        style = new StyleRange(i, 1, range.foreground, range.background, range.fontStyle | SWT.BOLD);
                    }
                    ;
        ...
        italicButton.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                for (int i = sel.x; i < sel.x + sel.y; i++) {
                    StyleRange range = styledText.getStyleRangeAtOffset(i);
                    if (range == null) {
                        style = new StyleRange(i, 1, null, null, SWT.ITALIC);
                    } else {
                        style = new StyleRange(i, 1, range.foreground, range.background, range.fontStyle | SWT.ITALIC);
                    }
                    ;
                    styledText.setStyleRange(style);
                }
