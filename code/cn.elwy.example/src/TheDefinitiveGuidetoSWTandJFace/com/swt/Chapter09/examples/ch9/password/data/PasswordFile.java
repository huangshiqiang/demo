package com.swt.Chapter09.examples.ch9.password.data;
...
import com.swt.Chapter09.examples.ch9.password.Password;
...
public class PasswordFile {
    ...
    public void save() throws IOException {
        if (filename == null) {
            throw new IllegalStateException("Cannot save file without a file name.");
        }
        ...
        if (password != null) {
            ...
            for (Iterator itr = categories.keySet().iterator(); itr.hasNext();) {
                ...
                for (int i = 0, n = entries.size(); i < n; i++) {
                    ...
                    // Encrypt the user ID
                    props.setProperty(name + USER_ID, SecurityHandler.encrypt(entry.getUserId(), password));

                    // Encrypt the password
                    props.setProperty(name + PASSWORD, SecurityHandler.encrypt(entry.getPassword(), password));
                }
            }
            ...
            throw new IllegalStateException("Cannot save file without a master password.");
        }
    }
    ...
    public void open() throws IOException {
        if (filename == null) {
            throw new IllegalStateException("Cannot open file without a file name.");
        }
        ...
        if (password != null) {
            // Verify the password
            if (SecurityHandler.checkHash(props.getProperty(HASH).getBytes(), SecurityHandler.getHash(password))) {
                ...
                for (Enumeration e = props.propertyNames(); e.hasMoreElements();) {
                    ...
                    if (key.endsWith(PASSWORD)) {
                        ...
                        try {
                            entries.add(new PasswordEntry(props.getProperty(key + CATEGORY), key, SecurityHandler
                                    .decrypt(props.getProperty(key + USER_ID), password), SecurityHandler.decrypt(
                                    props.getProperty(key + PASSWORD), password)));
